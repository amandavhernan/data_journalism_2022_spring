...
international_migration <- read_csv("tabula-md-statistical-handbook.csv", skip=2, col_names=c("state_region_county", "july_2011", "july_2012", "july_2013","july_2014", "july_2015", "july_2016", "july_2017", "july_2018", "july_2019", "total_int_migration"))
...
**A1** The Suburban Washington Region (62.8 -> 63%) and Montgomery County (36.7 -> 37%) accounted for the largest percentage of international migration between 2010-2019.

```{r}
# Write the code to produce your answer here

international_migration %>%
  mutate(percent_total = ((total_int_migration/198996)*100)) %>%
...
**A2** The data shows that the majority of those who migrated internationally settled in the Suburban Washington Region and roughly 37% of that population settled in Montgomery County.
...
**A3** The Southern Maryland Region saw a roughly -58% decrease in international migration between July 2018-2017 whereas Calvert County saw a -77% decrease.

```{r}
# Write the code to produce your answer here

international_migration %>%
  mutate(july_17_18_change = ((july_2018 - july_2017)/july_2017)*100) %>%
  arrange(july_17_18_change)
  
```

**Q4** What's your best guess as to why these changes occurred, and in those places in particular?
**A4** With Trump in office at the time, I know his administration cracked down on international immigration with policies such as the zero tolerance policy, which criminally prosecuted those who tried to cross U.S. boarders anywhere other than official ports.
...
domestic_migration <- read_csv("tabula-md-statistical-handbook-domestic.csv", skip=2, col_names=c("state_region_county", "july_2011", "july_2012", "july_2013","july_2014", "july_2015", "july_2016", "july_2017", "july_2018", "july_2019", "total_dom_migration"))
...
**A5** The Maryland jurisdiction with the largest net decrease in domestic migration was Baltimore County (-62834).

```{r}
# Write the code to produce your answer here

domestic_migration %>%
  arrange(total_dom_migration)
...
**A6** There were 14 regions and jurisdictions.

```{r}
# Write the code to produce your answer here

pos_migration <- domestic_migration %>%
  filter(july_2017 > 0 & july_2018 > 0 & july_2019 > 0) %>%
  select(state_region_county, july_2017, july_2018, july_2019)

```

**Q7** How would you describe this data? Is there a county or region that stands out, and why?
**A7** This data shows which counties and regions had a net positive increase over a three year period (2017-2019). The county that stood out the most to me was Talbot because domestic migration increased from 9 to 242, which is a larger jump compared to the other counties/regions. I also thought it was intresting how Caroline County went from 177 in 2017 to 9 in 2019.
