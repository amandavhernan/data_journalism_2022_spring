...
**Answer** The warning said there's either one or more parsing issues and that running problems() will provide details.
...
**Answer** No, there are no parsing errors.
...
**Answer** amount -> numeric (double), name -> character, id -> numeric (double)
```{r}
dplyr::glimpse(ppp_maryland_loans)
```
...
**Answer**: There are 192959 loans where the franchise_name is NA and 275 loans where the franchise_name is Subway.
...
**Answer**: There are 135,073 loans where the forgiveness_date is NA. And yes, it's the same number found for forgiveness_amount.
...
**Answer**: Yes, it's the same number. We can conclude that at least 135,073 loans were not forgiven, which explains why the forgiveness_amount and forgiveness_date fields are NA. It's possible that these loan recipients have already paid the loans off or chose not to apply for loan forgiveness.
...
**Answer** Zero loans were approved in October 2020. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan was $10000000 while the smallest was $6. I think both of these loans are a bit suspicious because the largest loan is also the maximum amount that the SBA was willing to distribute whereas the smallest seems to small to actually maintain payroll or any legitimate business-related expenses. With that in mind, I would want to investigate both further and see whether I can confirm any of the loan application information.
```{r}
...
**Answer** There are only two loans under $100.
```{r}
...
**Answer** Yes, all of the fields align with each other. 
...
**Answer** The first and second row of this data frame are duplicated. The style for the ZIP codes is inconsistent, three rows have the five digit ZIP codes and the other three have the nine digit ZIP codes. Another issue is that the city name, Arnold, is misspelled in some of the rows and the style is inconsistent.
...
**Answer** The function changed the original column name from "1_id" to "x1_id".
...
**Answer** The error said there's an issue with the summarise() function because the amount field's data type is invalid. The character data type can't be used with sum(). 
...
**Answer** There are two duplicates and the business name is Stuart Jones LLC. 
...
**Answer** The str_sub() function essentially adjusts the length a character vector/string.
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Arnold in the "city" field. How many misspellings of Arnold remain after running this code? Answer below.
**Answer** One misspelling
