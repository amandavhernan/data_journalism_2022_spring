...
**Answer** The code returns the top 10 counties with the most PPP loans in Maryland. The first step creates an object called "maryland_ppp_top_counties" and assigns the PPP dataframe to it. Then, the second step is to group the data by the "project_county_name" column. The next steps are to count the total number of loans (total_loans) and arrange it in descending order. The last line of code, head(10), returns the first 10 counties.
...
**Answer** The code returns an empty chart block. The ggplot() function takes certain arguments that will determine what kind of chart the code will generate and what it looks like.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** A basic bar chart with x and y labels for total number of loans (x) and county name (y). The county names are cluttered. 
...
**Answer** A bar chart reordered by county and total number of loans. The county name labels are still cluttered. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The coord_flip() function flipped the axes. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The theme_minimum() function removed the grey background. 
...
**Answer** The theme_economist() function added a new theme to the chart. The theme made the background light blue and changed the color of the grid lines to white. The label text size is also bigger and a darker, black color.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The bar chart now boasts a proper title, "Maryland Counties with Most PPP Loans," and new label names. The chart also has a source line. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code returns a dataframe with PPP data summarized by the total number of loans by the data the applications were approved.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code returns a line graph that represents the total number of loans approved over time.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The line graph has a new x-axis scale by month, which adds new, cluttered text labels.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** This code changes the x-axis' styling by adding an angle and height adjustment. This made the x-axis labels much more readable.
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
