...
**Answer** I see a screenshot of some files related to hospitals. I also see file-specific information such as date modified, size, and kind.
...
**Answer** This code returns a dataset with 33 columns. Some of the columns provide more basic such as name, address, county and state FIPS codes, latitude, and longitude. Some more interesting columns provide information about the number of beds each hospital has and whether each hospital has a trauma department or a helipad.
...
**Answer** The code returns a subset of the original dataframe that shows hospitals located in Maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns the total count of each type of hospital in Maryland. According to the output, the majority of hospitals (50 total) provide general acute care. The second and third most common hospital type are psychiatric and military respectively. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a dataset looking at general acute care hospitals in Maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns points where Maryland hospitals are located based on latitude and longitude. 
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code loads the tigris library.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns the names of all U.S. counties with additional geographic information that can be used for mapping.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a subset of Maryland counties from the original U.S. counties dataset.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a map of Maryland divided by county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a map of hospitals located in Maryland. Each point on the map represents a hospital.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns what seems like the same map from the previous code block, but this map color coded the hospital points by the number of beds using the "magma" color palette. The map itself is also filled in white instead of the default grey. The points are also a slightly bigger size.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns the dataset containing information about Maryland hospitals.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a tibble with total hospital bed counts by county in Maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a new dataset called "md_county_population," which contains population information for each Maryland county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns the same dataset as above, but it now includes a column called "geometry," which provides coordinates for mapping.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The codeblock returns the "md_county_population" and "md_bed_by_county" dataframes. Before joining, we need to standardize the names of the counties and rename the name/county columns so they're the same.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code standardized the md_county_population dataframe by changing the original name column to county, capitalizing the county names, and removing "County" and "Maryland." This new dataframe now only includes county, population, and geometry columns.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a new dataframe that was made by joining the md_county_population and md_beds_by_county dataframes. Now, the colums are county, population, total_beds, and geometry.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code removed the missing/null values, N/A, in the total_beds column and replaced them with zero.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This code calculated total number of beds per 100k people in each county and added a new column to the dataframe to house that information. The dataframe is also now arranged in descending order by the new column, "beds_per_100k."
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns a map of Maryland counties color coded by the total number of hospital beds per 100k people.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code returns the same map as above, but now featuring the magma color scale.
...
**Answer** The code returns the same map, but with a different scale, log. I noticed that the Eastern Shore and Western Maryland has more hospital beds per 100k people compared to other regions. Baltimore City seems to be the only exception. Southern Maryland also has the fewest number of total hospital beds per 100k people.
