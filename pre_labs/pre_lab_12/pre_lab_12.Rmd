...
**Answer** The dataframe contains 13 variables/columns and 11,397 records. 
...
**Answer** The dataframe contains each Maryland County and the total number of days each jurisdiction has vaccination for. Under the county column, there are rows titled "Unknown" and N/A," which is pretty unusual.
...
**Answer** The "N/A" records represent vaccinations where officials either don't know where the vaccinated person is from or where the vaccination took place. It's possible there were data entry errors or that information wasn't included during vaccination registration.
...
**Answer** It appears the number of vaccinations in the county was steadily decreasing.
...
**Answer** The chart shows vaccination rates skyrocketing during the first half or so of 2021 and then remaining at a more constant rate toward the end of the year.
...
**Answer** The code returns a subtitle that prompts the chart to automatically update graph's subtitle based on whether vaccinations have increased or decreased. The value of "sub" is "The number of people in Prince George's County who got fully vaccinated on March 06 decreased by -73.77 percent compared to the week before."
...
**Task** Run the following code and describe what the resulting chart shows
**Answer** The code returned a similar chart, but this one is using a log scale and scientific notation for the y-axis.
...
**Answer** The map seems to show the number of total COVID cases by county. Based on the legend/scale, counties shaded yellow or orange have had the most cases whereas counties shaded blue or purple have had significantly less in comparison.
