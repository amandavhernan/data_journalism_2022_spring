...
**Answer** The dataframe has 51 records/rows and five variables/columns. 
```{r}
...
**Answer** This scatterplot illustrates a weak positive linear relationship between the percentage of Trump votes in each state and the percentage of small businesses in each state that received a PPP loan.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This scatterplot shows a weaker positive linear relationship between the amount of PPP loan funding per employee and the percentage of Trump votes in each state.
...
**Answer** The output shows a moderate correlation, 0.52, for the "businesses_receiving_ppe_pct" variable and a weak correlation, 0.22, for the "ppe_amount_per_employee" variable.
```{r}
reveal_data %>%
...
**Answer** The model output returned a p-value of 0.00008607 at a 95 percent confidence interval. At an alpha level of 0.05, the p-value is less than 0.05, which means the result is statistically significant.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The model output returned a p-value of 0.1189 at a 95 percent confidence interval. Since the p-value is greater than 0.05 (alpha), the result is not statistically significant.
```{r}
...
**Answer** The table has two rows and three variables. The table compares the average PPP loan amount per employee and the average percentage of businesses receiving PPP loans between Clinton and Trump voters.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The t-est returned a t-value of 1.2223 and a p-value of 0.2295. The difference in means between the Clinton and Trump groups is ~1, which isn't that strong nor significant. The p-value is also greater than 0.05, which means it's not statistically significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The t-test returned a t-value of 3.182 and a p-value of 0.002643. The difference in means between the Clinton and Trump groups is high at ~3, which means it's significant. The p-value is also less than 0.05, which means it's statistically significant.
```{r}
