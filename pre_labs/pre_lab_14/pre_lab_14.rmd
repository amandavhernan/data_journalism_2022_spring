...
**Answer** The dataframe is composed of 12 columns/variables and 152,584 rows/observations.
...
**Answer** The unique functions returns the same list with duplicates removed, only the distinct words or characters appear in the output.
...
**Answer** The codeblock returned a dataframe containing unique words that appeared in the "content" variable/column in the "covid_tweets" dataframe.
...
**Answer** The code returns a bar chart displaying the frequency of the unique words found in tweets posted by federal politicians. Unfortunately, it's not that interesting because it includes boring filler words such as "the" or "and."  
...
**Answer** The code returns a tibble of unique words excluding stop words. It's defintely a lot more interesting than the first list.
...
**Answer** The code returns two tibbles with the top 10 unique words in 2020 and 2021. Each tibble is composed of three variables, "word," "n" (count), and "percent."
...
**Task** Run the following code and describe the results.
**Answer** The code returns a tibble with three variables, "bigram," "n" (count), and "percent." The bigram variable contains the most popular two-word phrases in 2020.
...
**Task** Run the following code and describe the results.
**Answer** Similar to the tibble made in task 11, this tibble contains the top 10 most popular two-word phrases in 2021.
...
**Task** Run the following code and describe the results.
**Answer** The codeblock returns two tibbles that contain the top 10 two-word phrases in 2020. Each tibble corresponds to tweets posted by either Democratic or Republican federal politicians.
...
**Task** Run the following code and describe the results.
**Answer** The codeblock returns two tibbles that contain the top 10 two-word phrases in 2020 and 2021 among Republican House candidates. The phrases in the first tibble seem fairly generic and represent what the first year in the pandemic was like in terms of concerns about nursing homes and social distancing mandates. The phrases in the second tibble, on the other hand, are a bit different and appear to show some of the progress that was made in 2021 with vaccines. Something I found off putting, though, was the phrase "illegal immigrants." I'm not entirely sure what that's about, but I would want to look into that further.
...
**Answer** The code returns two dataframes composed of unique words and their respective sentiments for 2020 and 2021. I thought it was strange how the word "Trump" had a positive sentiment for both years. I know it might be referring to the actual word and not the former president, but I would want to further analyze the context to make sure.
